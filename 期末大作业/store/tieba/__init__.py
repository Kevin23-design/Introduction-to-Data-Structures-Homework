...
async def update_tieba_note_comment(note_id: str, comment_item: TiebaComment):
    """
    Add or Update tieba note comment
    Args:
        note_id:
        comment_item:
    ...
    """
    comment_item.source_keyword = source_keyword_var.get()
    save_comment_item = comment_item.model_dump()
    save_comment_item.update({"last_modify_ts": utils.get_current_timestamp()})
    utils.logger.info(f"[store.tieba.update_tieba_note_comment] tieba note comment: {save_comment_item}")
