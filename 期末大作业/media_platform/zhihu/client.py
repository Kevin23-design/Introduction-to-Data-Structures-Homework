...
class ZhiHuClient(AbstractApiClient):
    ...
    async def get_note_all_comments(
        ...
        offset: str = ""
        limit: int = 10
        previous_offset: str = ""  # 用于检测重复的offset
        max_pages: int = 100  # 最大页数限制,防止无限循环
        page_count: int = 0
        ...
        while not is_end and page_count < max_pages:
            ...
            is_end = paging_info.get("is_end", False)  # 默认False,确保有明确的结束标志
            new_offset = self._extractor.extract_offset(paging_info)
            
            # 检测重复offset,防止无限循环
            if new_offset == previous_offset and new_offset != "":
                utils.logger.warning(f"[ZhihuClient.get_note_all_comments] Detected duplicate offset: {new_offset}, breaking loop")
                break
        ...
        if page_count >= max_pages:
            utils.logger.warning(f"[ZhihuClient.get_note_all_comments] Reached max pages limit ({max_pages}) for content {content.content_id}")
