...
class SentimentPlatformEnum(str, Enum):
    """情感分析支持的平台枚举"""
...
class AnalyzeIntervalEnum(str, Enum):
    """分析时间间隔枚举"""
...
async def parse_cmd(argv: Optional[Sequence[str]] = None):
    ...
    def main(
        ...
        sentiment: Annotated[
            Optional[SentimentPlatformEnum],
            typer.Option(
                "--sentiment",
                help="对指定平台执行情感分析 (zhihu=知乎 | xhs=小红书 | tieba=贴吧 | all=全部)",
                rich_help_panel="情感分析",
            ),
        ] = None,
        sentiment_type: Annotated[
            ...
            typer.Option(
                "--sentiment-type",
                help="情感分析类型 (post=帖子 | comment=评论 | all=全部)",
                rich_help_panel="情感分析",
            ),
        ] = SentimentTypeEnum.ALL,
        overwrite: Annotated[
            bool,
            typer.Option(
                "--overwrite",
                help="覆盖已有的情感分析结果，重新分析所有数据",
                rich_help_panel="情感分析",
            ),
        ] = False,
        analyze_sentiment: Annotated[
            bool,
            typer.Option(
                "--analyze-sentiment",
                help="执行情感趋势分析和可视化",
                rich_help_panel="数据分析",
            ),
        ] = False,
        analyze_platforms: Annotated[
            str,
            typer.Option(
                "--analyze-platforms",
                help="要分析的平台，多个用逗号分隔 (zhihu,xhs,tieba)",
                rich_help_panel="数据分析",
            ),
        ] = "zhihu,xhs,tieba",
        analyze_keywords: Annotated[
            str,
            typer.Option(
                "--analyze-keywords",
                help="关键词过滤，多个用逗号分隔（留空表示不过滤）",
                rich_help_panel="数据分析",
            ),
        ] = "",
        start_date: Annotated[
            ...
            typer.Option(
                "--start-date",
                help="开始日期 (格式: YYYY-MM-DD，留空为30天前)",
                rich_help_panel="数据分析",
            ),
        ] = "",
        end_date: Annotated[
            ...
            typer.Option(
                "--end-date",
                help="结束日期 (格式: YYYY-MM-DD，留空为今天)",
                rich_help_panel="数据分析",
            ),
        ] = "",
        analyze_interval: Annotated[
            AnalyzeIntervalEnum,
            typer.Option(
                "--analyze-interval",
                help="时间间隔 (day=天 | week=周 | month=月)",
                rich_help_panel="数据分析",
            ),
        ] = AnalyzeIntervalEnum.DAY,
        analyze_content_type: Annotated[
            SentimentTypeEnum,
            typer.Option(
                "--analyze-content-type",
                help="分析内容类型 (post=帖子 | comment=评论 | all=全部)",
                rich_help_panel="数据分析",
            ),
        ] = SentimentTypeEnum.ALL,
        ...
        sentiment_platform = sentiment.value if sentiment else None
        sentiment_type_value = sentiment_type.value
        
        # 处理分析参数
        analyze_platforms_list = [p.strip() for p in analyze_platforms.split(',') if p.strip()] if analyze_platforms else []
        analyze_keywords_list = [k.strip() for k in analyze_keywords.split(',') if k.strip()] if analyze_keywords else None
        analyze_interval_value = analyze_interval.value
        analyze_content_type_value = analyze_content_type.value
        ...
        return SimpleNamespace(
            ...
            sentiment=sentiment_platform,
            sentiment_type=sentiment_type_value,
            overwrite=overwrite,
            analyze_sentiment=analyze_sentiment,
            analyze_platforms=analyze_platforms_list,
            analyze_keywords=analyze_keywords_list,
            start_date=start_date or None,
            end_date=end_date or None,
            analyze_interval=analyze_interval_value,
            analyze_content_type=analyze_content_type_value,
